    ...
    <div className="gx-layout-sider-header">
        ...
        <div className="gx-linebar">
          <i
            className={`gx-icon-btn icon icon-${!sidebarCollapsed ? 'menu-unfold' : 'menu-fold'} ${
              themeType !== THEME_TYPE_LITE ? 'blue' : ''
            }`}
            onClick={() => {
      ...
      <Link to="/" className="gx-site-logo">
        {navStyle === NAV_STYLE_NO_HEADER_MINI_SIDEBAR && width >= TAB_SIZE ? (
          <img alt="lo" src={'/assets/images/images.png'} />
        ) : themeType === THEME_TYPE_LITE ? (
          <img alt="logo1" src={'/assets/images/images.png'} />
        ) : (
          <img alt="logo2" src={'/assets/images/images.jpg'} />
        )}
      </Link>
    </div>
  );
